...
const UpdateItem = (props) => {
  ...
  const [itemToUpdate, setItemToUpdate] = useState("1");  
  ...
  function UpdatePressed() {
    fetch(props.apiURI + '/items/'+itemToUpdate, {
      method: 'PUT',
      body: JSON.stringify({
        title: title,
  ...
  return (
    <View style={ styles.screen }>
      <ScrollView>
        <Text style={ styles.header }>Update an new item</Text>
        <Text>Please enter the reference of the item</Text>
        <TextInput style={ styles.input } value={ itemToUpdate } onChangeText={ value => setItemToUpdate(value)}/>
        ...
        <TouchableOpacity style={ styles.buttonUpdate } onPress={ () => UpdatePressed() }>
